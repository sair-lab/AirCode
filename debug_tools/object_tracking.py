...
def draw_object(data, colors=None):
  image = data['image']
  points = data['points']
  ...
  masks = objects['masks']
  if colors is None:
    colors = postprocess.compute_colors_for_labels(labels).tolist()
  
  image = postprocess.overlay_boxes(image, boxes, labels)
  # image = postprocess.overlay_mask(image, masks, labels)
  ...
  for p, c in zip(points, colors):
    ...
    for i in range(len(p)):
      ...
      if x < 0:
        continue
      cv2.circle(image, (x,y), 2, tuple(c), thickness=-1)
...
def draw_results(tpl_data, data, save_dir, image_name, match_thr=0.95):
  ...
  index = [i+1 for i in range(sum_num)]
  index = torch.tensor(index)
  sum_colors = postprocess.compute_colors_for_labels(index).tolist()
  ...
  # draw object
  tpl_image, _ = draw_object(tpl_data, tpl_colors)
  image, _ = draw_object(data, colors)
